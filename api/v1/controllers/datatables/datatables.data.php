...
class DatatablesData {
    ...
    static function selectTriviaHosts() {
        return DBConn::selectAll("SELECT DISTINCT(CONCAT(h.id,'-',v.id)) as temp,h.id,h.trv_users_id,u1.name_first AS nameFirst,u1.name_last AS nameLast,u1.email, h.address, h.address_b AS addressb, "
            ...
            . "CONCAT(u1.name_first, ' ', u1.name_last) AS createdBy, "
            . "CONCAT(u2.name_first, ' ', u2.name_last) AS updatedBy, "
            . "hns.trivia_day AS venue_triviaDay, hns.trivia_time AS venue_triviaTime,"
            . "v.id as venue_id, v.name AS venue, v.address as venue_address, v.address_b AS venue_addressb, v.city As venue_city, v.state AS venue_state, v.zip AS venue_zip, "
            . "v.phone AS venue_phone, v.phone_extension AS venue_phoneExtension,v.created_by_user_type AS venue_createdByUserType,  v.website AS venue_website, v.facebook_url as venue_facebook, "
            . "v.logo as venue_logo, v.referral AS venue_referralCode, v.created AS venue_created, v.disabled AS venue_disabled, "
            . "CONCAT(u3.name_first, ' ', u3.name_last) AS venue_contactUser, u3.id AS venue_contactUserId, "
            . "CONCAT(u4.name_first, ' ', u4.name_last) AS venue_createdBy, u4.id AS venue_createdById "
            ...
            . "LEFT JOIN " . DBConn::prefix() . "venues AS v ON hv.venue_id = v.id "
            . "LEFT JOIN " . DBConn::prefix() . "hosts_trivia_nights AS hns ON hns.host_id = h.id and hns.venue_id=hv.venue_id "
            . "LEFT JOIN " . DBConn::prefix() . "venue_roles AS vr ON vr.venue_id = v.id "
            . "LEFT JOIN " . DBConn::prefix() . "users AS u3 ON u3.id = vr.user_id "
            . "LEFT JOIN " . DBConn::prefix() . "users AS u4 ON u4.id = v.created_user_id "
            . "GROUP BY temp ORDER BY h.id;");
    }
