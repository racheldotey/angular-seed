...
class VenueController {
    ...
    static function getVenueList($app) {
        ...
        $venue = VenueData::getallVenue();
        if($venue) {
            return $app->render(200, array('venues' => $venue));
        } else {
            return $app->render(400,  array('msg' => 'No venues found.'));
        }
    }    
    ...
    static function addVenue($app) {
        ...
        if($venueId) {
            if((v::key('createdByUserType', v::stringType()->length(1,5))->validate($post)))
            {
                $assignment=array(
                    ":created_by_user_type"=>$post['createdByUserType'],
                    ":id"=>$venueId
                    );
    ...
    static function venue_validateVenueParams($post) {
        ...
        {
            $result = array('error' => true, 'msg' => 'Invalid joint parameters. Check your input and try again.');
    ...
    static function venue_getVenueArray($post) {
