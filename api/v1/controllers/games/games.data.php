...
class GameData {
    ...
    static function selectGame($gameId) {
        ...
        $qTeams = DBConn::executeQuery("SELECT t.id AS teamId, t.name, IFNULL(g.score, 0) AS gameScore, IFNULL(game_rank, 0) AS gameRank, game_winner AS gameWinner,COALESCE(c.sequence,0) AS teamSequence  "
            . "FROM " . DBConn::prefix() . "game_score_teams AS g "
            . "JOIN " . DBConn::prefix() . "teams AS t ON g.team_id = t.id "
			. "LEFT JOIN " . DBConn::prefix() . "logs_game_checkins AS c ON c.team_id = t.id AND c.game_id =  g.game_id "
    ...
    static function getTeamSequenceInGame($gameId) {
        return DBConn::selectColumn("SELECT sequence FROM " . DBConn::prefix() . "logs_game_checkins "
            . "WHERE game_id=:game_id  order by sequence desc  LIMIT 1;", array(':game_id' => $gameId));
    }
    private static function insertCheckinLog($teamId, $gameId, $userId, $status) {

        $sequence=self::getTeamSequenceInGame($gameId);
        if(!$sequence){
            $sequence=1;
        }
        else{
        ...
        $qLog = DBConn::preparedQuery("INSERT INTO " . DBConn::prefix() . "logs_game_checkins(team_id,game_id,status,created_by,sequence) "
            . "VALUES(:team_id,:game_id,:status,:created_by,:sequence);");
        if($teamId) {
            $qLog->execute(array(
                ':team_id' => $teamId,
                ':game_id' => $gameId,
