...
class GameData {
    ...
    static function selectGame($gameId, $roundNumber = 1) {
        $game = DBConn::selectOne("SELECT g.id, g.name, g.scheduled, g.venue_id AS venueId, g.host_user_id AS hostId, "
                ...
                . "FROM as_games AS g "
                . "LEFT JOIN as_users AS u ON u.id = g.last_updated_by "
                . "LEFT JOIN as_users AS h ON h.id = g.last_updated_by "
    ...
    static function selectGameRound($gameId, $roundNumber) {
            ...
            if($round) {
                ...
                $qRoundTeams = DBConn::executeQuery("SELECT t.name AS team, s.team_id AS teamId, "
                        ...
                        . "LEFT JOIN as_teams AS t ON t.id = s.team_id "
                        . "LEFT JOIN as_game_score_rounds AS r ON s.team_id = r.team_id AND r.round_id = :round_id "
    ...
    static function selectRound($roundId) {
            ...
            $round = DBConn::selectOne("SELECT r.id AS roundId, r.game_id AS gameId, r.order AS roundNumber, r.name, r.max_points AS maxPoints, r.default_question_points AS defaultQuestionPoints "
                    . "FROM as_game_rounds AS r WHERE r.id = :id;", 
                    array(':id' => $roundId));
    ...
    static function getRoundCount($gameId) {
        return DBConn::selectColumn("SELECT COUNT(id) AS count FROM as_game_rounds "
                . "WHERE game_id=:game_id LIMIT 1;", array(':game_id' => $gameId));
    }
    
    static function getQuestionCount($roundId) {
        return DBConn::selectColumn("SELECT COUNT(id) AS count FROM as_game_round_questions "
    ...
    static function insertStartingRounds($count, $gameId, $defaultPoints, $currentUser) {
        $qAddRound = DBConn::preparedQuery("INSERT INTO as_game_rounds(name, `order`, game_id, default_question_points, created_user_id, last_updated_by) "
    ...
    static function insertRound($validRound) {
        $results = DBConn::insert("INSERT INTO as_game_rounds(name, `order`, game_id, default_question_points, created_user_id, last_updated_by) "
    ...
    static function insertQuestion($validQuestion) {
        $results = DBConn::insert("INSERT INTO as_game_round_questions(question, `order`, game_id, round_id, max_points, created_user_id, last_updated_by) "
    ...
    static function selectStarted($gameId) {
        return DBConn::selectColumn("SELECT game_started FROM as_games "
    ...
    static function selectEnded($gameId) {
        return DBConn::selectColumn("SELECT game_ended FROM as_games "
                . "WHERE id=:id LIMIT 1;", array(':id' => $gameId));
    }
    ...
    static function saveQuestionScores($scores) {
        $saveQuestioneScore = DBConn::preparedQuery("INSERT INTO as_game_score_questions(game_id, round_id, question_id, team_id, score, created_user_id) "
    ...
    static function calculateRoundScores($gameId, $currentUser) {
        ...
        $teams = DBConn::selectColumn("SELECT team_id FROM as_game_score_teams "
                . "WHERE game_id=:game_id;", $game);
        
        $rounds = DBConn::selectColumn("SELECT id FROM as_game_rounds "
                 . "WHERE game_id=:game_id;", $game);
        
        $saveRoundScore = DBConn::preparedQuery("INSERT INTO as_game_score_rounds (game_id,round_id,team_id,created_user_id) "
        ...
        $result[] = DBConn::update("UPDATE as_game_rounds AS r "
                . "INNER JOIN (SELECT qs.round_id, SUM(qs.max_points) AS maxpoints "
                . "FROM as_game_round_questions AS qs GROUP BY qs.round_id) AS q ON r.id = q.round_id "
                . "SET r.max_points = q.maxpoints WHERE r.game_id = :game_id;", $game);
        
        
        // Update Team Totals for each Round
        $result[] = DBConn::update("UPDATE as_game_score_rounds AS r "
                . "INNER JOIN (SELECT qs.round_id, qs.team_id, SUM(qs.score) AS total "
                . "FROM as_game_score_questions AS qs GROUP BY qs.round_id, qs.team_id) AS q "
        ...
        $result[] = DBConn::update("SET @lastscore = 0; SET @ordering = 0; "
                . "UPDATE as_game_score_rounds SET round_rank = "
