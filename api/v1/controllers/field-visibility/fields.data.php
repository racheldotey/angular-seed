...
class FieldData {
    ...
    static function getField($id) {
        $field = DBConn::selectOne("SELECT f.id, f.identifier, f.type, f.desc, f.initialized, f.created, f.last_updated AS lastUpdated, "
                ...
                . "FROM as_auth_fields AS f "
                . "JOIN as_users AS u1 ON u1.id = f.created_user_id "
                . "JOIN as_users AS u2 ON u2.id = f.last_updated_by "
    ...
    static function getByIdentifier($identifier, $fieldId = 0) {
        $field = DBConn::selectOne("SELECT f.id, f.identifier, f.type, f.desc, f.initialized, f.created, f.last_updated AS lastUpdated, "
                . "CONCAT(u1.name_first, ' ', u1.name_last) AS createdBy, CONCAT(u2.name_first, ' ', u2.name_last) AS updatedBy "
                . "FROM as_auth_fields AS f "
                . "LEFT JOIN as_users AS u1 ON u1.id = f.created_user_id "
                . "LEFT JOIN as_users AS u2 ON u2.id = f.last_updated_by "
    ...
    static function insertField($validField) {
        return DBConn::insert("INSERT INTO as_auth_fields(identifier, type, desc, created_user_id, last_updated_by) "
                . "VALUES (:identifier, :type, :desc, :created_user_id, :last_updated_by)", $validField);
    }
    
    static function updateFieldInitialize($validField) {
        return DBConn::update("UPDATE as_auth_fields SET initialized=:initialized WHERE id=:id;", $validField);
    }
    
    static function updateField($validField) {
        return DBConn::update("UPDATE as_auth_fields SET identifier=:identifier, type=:type, "
                . "desc=:desc, last_updated_by=:last_updated_by, initialized=NULL WHERE id=:id;", $validField);
    }
    
    static function deleteField($id) {
        $roles = DBConn::delete("DELETE FROM as_auth_lookup_role_field WHERE auth_field_id = :id;", array('id' => $id));
        
        return (!$roles)  ? false :
            DBConn::delete("DELETE FROM as_auth_fields WHERE id = :id LIMIT 1;", array('id' => $id));
    }
  
    static function insertRoleAssignment($data) {
        return DBConn::insert("INSERT INTO as_auth_lookup_role_field(auth_field_id, auth_role_id, created_user_id) "
                . "VALUES (:auth_field_id, :auth_role_id, :created_user_id)", $data);
    }
                    
    static function deleteRoleAssignment($data) {
        return DBConn::delete("DELETE FROM as_auth_lookup_role_field "
    ...
    static function selectVisibilityKey() {
        ...
        $qElements = DBConn::executeQuery("SELECT id, identifier, initialized "
                . "FROM as_auth_fields WHERE initialized = 1 ORDER BY identifier;");
        
        $qRoles = DBConn::preparedQuery("SELECT auth_role_id AS id "
                . "FROM as_auth_lookup_role_field "
    ...
    static function updateVisibilityElementInit($identifier) {
        return DBConn::update("UPDATE as_auth_fields SET initialized=NOW(), last_updated_by=:last_updated_by "
